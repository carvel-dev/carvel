...
Additionally, You can use `kapp.k14s.io/versioned-keep-original` annotation in conjunction with `kapp.k14s.io/versioned` to have the latest version of original resource (resource without `-ver-{n}` suffix in name) along with versioned resource.
